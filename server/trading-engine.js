export class TradingEngine {
  ...
  start(balance) {
    ...
    this.dayStartBalance=value;
    this.pnl=0; this.trades=0; this.consecutiveLosses=0; this.stopped=false;
  ...
  finalizeOrder(key,boughtContracts=0) {
    ...
    this.trades+=Number(boughtContracts)||0;
    return {ok:true};
  }
  ...
  addContract(id,meta={}) {
    this.active.set(String(id),{...meta,id:String(id),startedAt:Date.now()});
  }
  ...
  settle(id,profit) {
    ...
    this.pnl+=p;
    if(p<0) this.consecutiveLosses++;
    else if(p>0) this.consecutiveLosses=0;
  ...
  status(){
    return {
      ...
      activeContracts:[...this.active.values()],
      reservedContracts:this.reservedCount()
    };
  }
}
