...
if(run&&m==="AUTO"&&next.direction!=="WAIT"&&Date.now()-lastAction.current.time>5000&&next.direction!==lastAction.current.direction){lastAction.current={direction:next.direction,time:Date.now()};paperTrade(next.direction);}
if(run&&m==="SEMI-AUTO"&&next.direction!=="WAIT"&&Date.now()-lastAction.current.time>5000&&next.direction!==lastAction.current.direction){lastAction.current={direction:next.direction,time:Date.now()};setPending({direction:next.direction,strength:next.strength,price});}};return()=>w.close()},[run,m]);
function paperTrade(d){const stake=Number(s)||0,contracts=Number(c)||1,win=Math.random()>.48,v=win?stake*contracts*.92:-stake*contracts;setB(z=>z+v);setTs(q=>[{d,win,v,mode:m,strength:signal.strength},...q].slice(0,15));}
...
<section><div className="panel"><h2>Trading Controls</h2><div className="tabs">{["MANUAL","SEMI-AUTO","AUTO"].map(x=><button key={x} className={m===x?"on":""} onClick={()=>{setM(x);setPending(null)}}>{x}</button>)}</div>
{m==="SEMI-AUTO"&&pending&&<div className="approval"><div><strong>Strategy signal: {pending.direction}</strong><small>Confidence {pending.strength}% · Price {pending.price}</small></div><div><button className="approve" onClick={()=>{paperTrade(pending.direction);setPending(null)}}><Check/> APPROVE</button><button className="reject" onClick={()=>setPending(null)}>REJECT</button></div></div>}
...
<p>Strategy signals are informational and paper execution remains enabled. Real-money execution is disabled.</p></div>
